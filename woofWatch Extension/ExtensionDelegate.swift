...
class ExtensionDelegate: NSObject, WKExtensionDelegate {
    public private(set) var data: [DisplayValue] = []

    func insert(data item: DisplayValue) {
        if data.count > 128 {
            data = Array(data[data.count - 128 ..< data.count])
        }
        if data.isEmpty {
            data.append(item)
        } else {
            for idx in (0 ..< data.count).reversed() {
                if data[idx].date < item.date {
                    data.insert(item, at: idx + 1)
                    break
                } else if data[idx].date == item.date {
                    break
                }
            }
        }
